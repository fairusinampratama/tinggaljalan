...
export function Hero({ t, language, booking, setBooking, whatsappUrl }) {
  ...
  const paxOptions = publicData.bookingOptions?.paxOptions ?? [];
  const selectedDestination = publicData.destinations?.find((destination) => destination.name === booking.destination);
  const routeSearchPath = selectedDestination ? `/routes?destination=${encodeURIComponent(selectedDestination.slug ?? selectedDestination.name)}` : '/routes';
