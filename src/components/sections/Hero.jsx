import { Globe2, MessageCircle, Route, Search } from 'lucide-react';
    ...
    <section id="home" className="relative overflow-visible bg-white">
      <div className="relative min-h-[620px]">
        ...
        <div className="relative mx-auto flex min-h-[620px] max-w-7xl items-center px-4 pb-28 pt-20 text-white sm:px-8 sm:pt-24 lg:px-10">
          <div className="max-w-3xl">
            <p className="mb-5 inline-flex items-center gap-2 rounded-full bg-white px-4 py-2 text-xs font-black uppercase tracking-[0.14em] text-brandBlue">
              <Globe2 className="h-4 w-4" /> {t.heroTag}
            </p>
            <h1 className="font-display text-4xl font-black leading-[0.98] sm:text-6xl lg:text-7xl">{t.heroTitle}</h1>
            <p className="mt-6 max-w-2xl text-base font-semibold leading-8 text-white/90 sm:text-lg">{t.heroText}</p>
      ...
      <div className="relative z-30 mx-auto -mt-10 max-w-6xl px-4 pb-28 sm:px-8 sm:pb-32 lg:px-10">
        <div className={`rounded-2xl border border-brandLine bg-white p-4 shadow-soft sm:p-5 ${cardHoverClass}`}>
          <p className="mb-4 font-display text-2xl font-black text-brandDark">{t.searchTitle}</p>
