...
class PaymentRequestWhatsAppMessage
{
    public function text(BookingPayment $payment): string
    {
        ...
        if (($payment->quote_currency ?: 'IDR') === 'USD') {
            ...
            $chargeLabel = $payment->provider === 'midtrans'
                ? $t('midtrans_charge')
                : $t('charge_label', ['provider' => $providerLabel]);
            $lines[] = $chargeLabel.': '.PublicSite::formatMoney($payment->charge_amount, 'IDR');
        ...
        return implode("\n", [...$lines, $t('pay_before').": {$expiresAt}", '', '*'.$t('pay_securely', ['provider' => $providerLabel]).'*', $paymentUrl, '', $t('booking_confirmed'), '', $t('need_help'), $t('security'), '', 'Tinggal Jalan']);
    }
